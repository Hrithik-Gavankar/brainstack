...
Config (any of env / flags / existing project.public.env):
  ...
  --db-url URL              Postgres URI to apply migrations (psql) — never written to project.public.env
...
Migration strategy (first match wins):
  ...
  2. linked CLI         → supabase db push (project already linked)
  3. --db-url           → psql applies each migration in timestamp order
  4. otherwise          → write combined SQL + actionable SQL Editor steps; use --skip-migrations after
...
JIRA_KEY=""
JIRA_TITLE=""
DB_URL="${TEAM_BRAIN_DB_URL:-${DATABASE_URL:-}}"
USE_LOCAL=0
SKIP_MIGRATIONS=0
SKIP_REGISTER=0
WRITE_ENV=0
